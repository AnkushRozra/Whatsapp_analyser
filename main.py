...
def process_chat(file_path):
    ...
    pd.df_summary = chat_summary(df)
    # print('------------------- Summary -------------------')
    print("-------------- Generating Graphs --------------")
    # pd.df_media = media_per_contact(df, save_path, sort=True, plot=True)
    pd.df_daily_msgs = daily_msgs(df)
    ...
    pd.df_msgs_per_year = msgs_per_year(df_extended, save_path, sort=False, plot=True)
    pd.df_emojis_per_user = emojis_per_user(df, save_path, sort=True, plot=True)
    pd.df_msgs_per_contact = msgs_per_contact(df_extended, save_path, sort=True, plot=True)
    pd.df_words_per_contact = words_per_contact(
        df_extended, save_path, sort=True, plot=True
    )

    pd.df_emojis_per_msg_per_contact = emojis_per_msg_per_contact(
        df_extended, save_path, sort=True, plot=True
    )
    if args.keyword != '':
        pd.df_this_word_per_contact = this_word_per_contact(
            df_extended,
